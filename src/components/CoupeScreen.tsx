    ...
    <div className="min-h-screen bg-radial-gradient p-4 md:p-6">
      <div className="text-center mb-6">
        <h1 className="text-2xl md:text-3xl font-bold font-heading text-foreground">
          CHAMPION 🏆 <span className="text-primary text-lg font-normal">— Phase de poules</span>
        </h1>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-4 lg:gap-6 max-w-[1400px] mx-auto">
        ...
        <div className="lg:flex-[1]">
          <div className="glass-card p-4">
            <h2 className="text-sm font-bold uppercase tracking-widest text-muted-foreground mb-3">Poules</h2>
  ...
  return (
    <div>
        ...
        <div className="glass-card p-8 animate-scale-up">
          ...
          <div className="flex items-center justify-center gap-4 mb-6 flex-wrap">
            <button
              ...
              className="bg-secondary hover:bg-primary hover:text-primary-foreground transition-all duration-200 rounded-xl px-6 py-5 text-foreground font-bold font-heading text-lg min-w-[100px]"
            >
              {activeMatch.player1}
            </button>
            <button
          ...
          <p className="text-xs text-center text-muted-foreground">Cliquez sur le vainqueur ou NUL</p>
        </div>
      ) : (
      ...
      <div className="mt-4 glass-card p-4">
        <h3 className="text-xs font-bold uppercase text-muted-foreground mb-2">Historique Poule {group.name}</h3>
        <div className="space-y-1 max-h-40 overflow-y-auto">
          {group.matches.filter(m => m.status === 'completed').map(m => (
            <div key={m.id} className="text-xs py-1 border-b border-border last:border-0">
              <span className={m.winner === m.player1 ? 'text-foreground font-bold' : 'text-muted-foreground'}>{m.player1}</span>
              <span className="text-muted-foreground">{m.winner === 'draw' ? ' = ' : ' vs '}</span>
          ...
          <tbody>
            {group.standings.map((s, i) => {
              ...
              return (
                ...
                >
                  <td className="py-1.5 font-bold text-xs">{i + 1}</td>
                  <td className="py-1.5 font-heading font-bold text-xs">
                    {s.player}
                    {isTied && <span className="text-accent ml-1 text-[10px]">(égalité)</span>}
                  </td>
                  <td className="py-1.5 text-center font-bold text-xs">{s.points}</td>
                  <td className="py-1.5 text-center text-xs">{s.wins}</td>
                  <td className="py-1.5 text-center text-xs">{s.draws}</td>
                  <td className="py-1.5 text-center text-xs">{s.losses}</td>
                  <td className="py-1.5 text-center text-muted-foreground text-xs">{s.played}</td>
                </tr>
              );
            })}
          </tbody>
